...
export class PlaceComponent implements OnInit {
  ...
  getWorkplaceByManager() {
    ...
    this.placeService.getAvailableByDate(
      this.userAccount.id, this.searchText, this.currentStatus, this.zoneId, `${from};${to}`, '', 'id', this.currentPage, 9)
      .then(
        (response: PlacePagination) => {
          this.placeList = response.listOfWorkplace.content;
          this.placeResponse = response.listOfWorkplace;
  ...
  getAllWorkplaceByManager() {
    ...
    this.showAll = true;
    this.placeService.getAvailableByDate(
      this.userAccount.id, this.searchText, this.currentStatus, '', `${from};${to}`, '', 'id', this.currentPage, 9)
      .then(
        (response: PlacePagination) => {
          this.placeList = response.listOfWorkplace.content;
          this.placeResponse = response.listOfWorkplace;
